...
(use-package org-roam
  ...
  :config
  ;; Définir org-roam-directory en utilisant getenv
  (setq org-roam-directory (getenv "RoamNotes"))
  ;; Définir les templates de capture en utilisant le chemin défini dans RoamNotes
  (setq org-roam-capture-templates
        `(("d" "default" plain
           ...
           :unnarrowed t)
          ("a" "article" plain
           (file ,(concat (getenv "RoamNotes") "/Templates/article-template.org"))
           :if-new (file+head "references/notes/${citekey}.org" "#+title: ${citekey}\n")
           :unnarrowed t)
          ("c" "code commandes" plain
           (file ,(concat (getenv "RoamNotes") "/Templates/code-commandes-template.org"))
           :if-new (file+head "%<%Y%m%d%H%M%S>-${slug}.org" "#+title: ${title}\n")
           :unnarrowed t)
          ("p" "projet" plain
           (file ,(concat (getenv "RoamNotes") "/Templates/projects-templates.org"))
           :if-new (file+head "%<%Y%m%d%H%M%S>-${slug}.org" "#+title: ${title}\n")
           :unnarrowed t)
          ("t" "théâtre" plain
           (file ,(concat (getenv "RoamNotes") "/Templates/théâtre-template.org"))
           :if-new (file+head "%<%Y%m%d%H%M%S>-${slug}.org" "#+title: ${title}\n")
           :unnarrowed t)))
  ;; Définir les templates de capture quotidienne
  ...
  :config
  ;; Initialiser org-roam
  (org-roam-setup)
  ;; Assurer la disponibilité du keymap pour org-roam-dailies
...
(defun my/org-roam-find-article ()
  ...
  (org-roam-node-find
   ...
   nil
   :templates
   `(("a" "article" plain (file ,(concat (getenv "RoamNotes") "/Templates/article-template.org"))
      :if-new (file+head "%<%Y%m%d%H%M%S>-${slug}.org" "#+title: ${title}\n#+category: ${title}\n")
      :unnarrowed t))))
...
(setq bibtex-completion-bibliography `(,(concat (getenv "RoamNotes") "/references/master.bib")))  ; location of .bib file containing bibliography entries
...
(setq bibtex-completion-library-path `(,(concat (getenv "RoamNotes") "/references/documents/")))  ; in this dir, %citekey-name(s).pdf would automatically attach pdf(s) to %citekey
(setq bibtex-completion-notes-path (concat (getenv "RoamNotes") "/references/notes/"))           ; dir to keep notes for the pdfs
...
(setq org-noter-notes-search-path `(,(concat (getenv "RoamNotes") "/references/notes/"))) ; V IMPORTANT: SET FULL PATH!
...
(use-package citar
  ...
  (citar-library-paths
   (list (concat (getenv "RoamNotes") "/references/documents/")))
  (citar-notes-paths
   (list (concat (getenv "RoamNotes") "/references/notes/")))
  (citar-templates
...
(defun my/org-copy-bibliographies ()
  ...
  (let* ((org-roam-dir (getenv "RoamNotes"))
    ...
    ;; Parcourir tous les fichiers Org
    (dolist (file files)
      (with-temp-buffer
        (insert-file-contents file)
        (goto-char (point-min))
        (when (re-search-forward "^#\\+filetags:.*:article:" nil t)
          ;; Trouver et copier la section Bibliographie
          (when (re-search-forward "^\\* Bibliographie" nil t)
            (let ((start (point)))
              (if (re-search-forward "^\\*\\*" nil t)
                  (setq bibliography-contents (concat bibliography-contents
    ...
    ;; Afficher le contenu copié pour débogage
    (message "Contenu copié: %s" bibliography-contents)
    
    ;; Tuer le buffer du fichier cible s'il est ouvert
...
;; Ajoutez cette ligne pour tester facilement la fonction
(my/org-copy-bibliographies)

(defun my/org-copy-to-cited-authors ()
  "Copy the selected text and append it to the 'Auteurs cités' section in an Org buffer.
Creates the section at the end of the document if it doesn't exist.
The first entry is added directly after the section title without a blank line."
  (interactive)
  (if (use-region-p)
      (let ((selected-text (buffer-substring-no-properties (region-beginning) (region-end))))
        (save-excursion
          ;; Rechercher ou créer la section "Auteurs cités"
          (goto-char (point-min))
          (unless (re-search-forward "^\\* Auteurs cités" nil t)
            ;; Créer la section à la fin du document si elle n'existe pas
            (goto-char (point-max))
            (unless (bolp) (newline)) ;; Ajouter un saut de ligne si nécessaire
            (insert "* Auteurs cités"))
          ;; Aller à la fin du titre de la section
          (re-search-forward "^\\* Auteurs cités" nil t)
          (end-of-line)
          ;; Vérifier si c'est la première entrée
          (if (looking-at-p "\n- ")
              (newline) ;; Si des entrées existent déjà, ajouter une nouvelle ligne
            (insert "\n")) ;; Sinon, rester directement après le titre
          ;; Ajouter l'entrée
          (insert "- " selected-text)
          ;; Afficher un message de confirmation
          (message "Texte copié dans la section 'Auteurs cités'.")))
    (message "Veuillez sélectionner du texte à copier.")))

;; Assigner C-c C-c à la fonction, uniquement en org-mode
(with-eval-after-load 'org
  (define-key org-mode-map (kbd "C-c C-c") #'my/org-copy-to-cited-authors))
